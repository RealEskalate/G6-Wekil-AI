...
const steps: Step[] = [
  {
    ...
    titleAmharic: "የውል አይነት ምረጥ",
    icon: FileText,
  },
  {
    id: 2,
    title: "Parties Information",
    titleAmharic: "የተሳታፊዎች መረጃ",
    icon: Users,
  },
  ...
  const validateStep = (step: number): boolean => {
    switch (step) {
      ...
      case 2:
        ...
        if (
          !emailRegex.test(firstParty.email) ||
          !emailRegex.test(secondParty.email)
        ) {
        ...
        break;
      case 3:
        const { amount, startDate, endDate, description } =
          contractData.dealTerms;
  ...
  return (
    <div
      className={`h-full flex flex-col bg-gray-100 ${
        currentLanguage === "amharic" ? "font-ethiopic" : ""
      }`}
    >
      {/* Header */}
      <div className="px-6 py-4 flex items-center justify-between">
        <div className="flex items-center space-x-4">
          <Button
            ...
            size="sm"
            onClick={() =>
              currentStep === 1 && onBackToDashboard
                ? onBackToDashboard()
                : handleBack()
            }
