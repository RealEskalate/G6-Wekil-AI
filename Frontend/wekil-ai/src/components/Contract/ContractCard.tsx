...
function getTimeAgo(createdAt: string, lang: "am" | "en"): string {
  ...
  if (diffYears > 0) {
    return `${diffYears} ${viewContractTranslation[lang].getTimeAgo.year} ${
      diffYears !== 1 && lang === "en" ? "s" : ""
    } ${translation.ago}`;
  } else if (diffMonths > 0) {
    return `${diffMonths} ${translation.month}${
      diffMonths !== 1 && lang === "en" ? "s" : ""
    } ${translation.ago}`;
  } else if (diffDays > 0) {
    return `${diffDays} ${translation.day}${
      diffDays !== 1 && lang === "en" ? "s" : ""
    } ${translation.ago}`;
  } else if (diffHours > 0) {
    return `${diffHours} ${translation.hour}${
      diffHours !== 1 && lang === "en" ? "s" : ""
    } ${translation.ago}`;
  } else if (diffMins > 0) {
    return `${diffMins} ${translation.minute}${
      diffMins !== 1 && lang === "en" ? "s" : ""
    } ${translation.ago}`;
  } else if (diffSecs > 0) {
    return `${diffSecs} ${translation.second}${
      diffSecs !== 1 && lang === "en" ? "s" : ""
    } ${translation.ago}`;
  } else {
    ...
    <div className="w-full bg-white col-span-2 md:col-span-1 border border-gray-200 rounded-2xl p-4 m-4">
      ...
      <div className="my-2">
        ...
        <p className="text-xs text-gray-500">
          {contract.type} {`${lang == "en" ? "Agreement" : "ውሎች"}`}
        </p>
      </div>
