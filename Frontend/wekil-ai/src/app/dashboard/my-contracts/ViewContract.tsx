...
const ViewContract: React.FC<ViewContractProps> = ({ contract }) => {
  ...
  return (
    <div className="border-l border-gray-200 min-h-screen p-5">
      <div className="md:flex md:justify-between mt-12 mx-4">
        <div className="grid grid-cols-1 md:grid-cols-2 md:gap-8">
          <span className="">
            <p className="text-2xl my-2 px-4 text-blue-950 font-semibold">
              {lang === "en" ? "Contract Details" : "የውል ዝርዝር"}
            </p>
            <p className="text-md pl-4 mb-2 text-gray-500 font-normal">
              {contract.title}
            </p>
          </span>
        </div>
      ...
      <div className="grid grid-cols-4 items-start">
        ...
        <div className="col-span-4 md:col-span-1 h-auto">
          <div className="border border-gray-200 bg-white  rounded-2xl py-4">
            <p className="text-center text-lg font-semibold">
              {lang == "en" ? "Quick Actions" : "ፈጣን "}{" "}
            </p>
            {options.map((item, index) => (
              <div
